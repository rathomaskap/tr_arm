...
extern struct dnat_sip_proxy_h  dnat_sip_proxy;
...
static pj_status_t init_options(int argc, char *argv[])
{
	...
	case 'a':
		...
		dnat_sip_proxy.src_net1 = (unsigned int) inp.s_addr;
	...
	case 'b':
		cp = pj_str(pj_optarg);
		ret =  pj_inet_aton	(&cp,&inp);
		dnat_sip_proxy.src_net2 = (unsigned int) inp.s_addr;
		if(!ret) return PJ_FALSE;
	    break;
	...
	case 'x':
		cp = pj_str(pj_optarg);
		ret =  pj_inet_aton	(&cp,&inp);
		dnat_sip_proxy.sip_ip1 = (unsigned int) inp.s_addr;
		if(!ret) return PJ_FALSE;
	    break;
	case 'y':
		cp = pj_str(pj_optarg);
