...
struct media_stream mstream[MAX_STREAMS];
...
{
    { { "PCMA", 4 }, 8, 8000, 1, 10, 0, 0 },
	{ { "PCMU", 4 }, 0, 8000, 1, 10, 0, 0 },
    { { "R2S", 3 }, 123, 8000, 1, 10, 0, 0 },
...
pj_status_t pjsua_aud_subsys_init()
{
    ...
    {
    mstream[i].samples_per_frame =1000/20;
    mstream[i].clock_rate =  codec_list[0].clock_rate;

    mstream[i].interval = mstream[i].samples_per_frame * 1000 / mstream[i].clock_rate;
    pjmedia_rtp_session_init(&mstream[i].out_sess,8, 0x0815+i);
    }
...
//    pjmedia_transport_send_rtp(call_med->tp, pkt, strlen(pkt));
    strm = &mstream[call_med->call->index];

    status = pjmedia_rtp_encode_rtp( &strm->out_sess, 123,
				     0, /* marker bit */
				     1600,
				     1600,
				     &p_hdr, &hdrlen);
    ...
    hdr = (pjmedia_rtp_hdr*) p_hdr;
    hdr->x = 1;  //Header Extension ist 1

    pj_memcpy(packet, hdr, hdrlen);

	/* Zero the payload */

    struct pjmedia_rtp_ext_hdr *exthdr = (void*) (packet+hdrlen);
    exthdr->profile_data = htons(0x0167);
    exthdr->length=htons(1);

    phext =  (unsigned int*) (packet+hdrlen+sizeof(pjmedia_rtp_ext_hdr));
    *phext = htonl(0xfffe0000);
	// pj_bzero(packet+hdrlen, 8);
...
//    pj_bzero(packet+12, 8);


	status = pjmedia_transport_send_rtp(call_med->tp,packet, hdrlen+8);
	if (status != PJ_SUCCESS)
	    printf("Error sending RTP packet\n");
    ...
    pjsua_schedule_timer2(&timer_to_send_aud_rtp, call_med, 200);
}
